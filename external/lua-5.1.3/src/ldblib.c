...
// const char* s = lua_tostring( L, 1 ); printf( "error msg(%s)\n", s ? s : "(null)" );
  ...
  if (lua_isfunction(L, -1)) {
	...
	if (ar->event == LUA_HOOKERROR && lua_isstring(L, 1)) {
      lua_pushvalue( L, 1 );
    }
	else lua_pushnil(L);
...
LUALIB_API void lua_traceback (lua_State *L, lua_State *L1, int level) {
  ...
  while (lua_getstack(L1, level++, &ar)) {
    ...
    // CL Fix: Use the full source file path if available
    lua_pushfstring(L, "%s:", (ar.source[0] == '@' ? ar.source+1 : ar.short_src));
    if (ar.currentline > 0)
